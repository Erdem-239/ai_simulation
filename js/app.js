...
/* ---- Yol Haritası: tech tree (v3: Civ6 yatay + çağlar + Zekapedia) ---- */
(function(){
  ...
  function show(id){
    ...
    btns+='<button class="addbtn" data-pedia="'+n.id+'" style="background:var(--accent); color:#101830">📖 Zekapedia&#39;da oku</button> ';
    ...
    info.innerHTML='<b style="font-size:16px">'+esc(n.nm)+'</b>'
      ...
      +'<div style="font-size:12px; color:var(--accent); font-weight:700; margin:12px 0 4px">👤 KEŞİF — Zekapedia</div>'
...
(function(){
  ...
  function applyTreeOrder(){
    ...
    seq.forEach(b=>{
      const m=b.dataset.model;
      const n=m ? api.NODES.find(x=>x.tab===m) : null;
      if(!n){ b.style.setProperty('--era-c', META); return; }   // ders değil (Yol Haritası/Zekapedia/RNN Test)
