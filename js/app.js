...
(function(){
  ...
  if(!rsR) return;
  let w=300;
  try{ const s=parseInt(localStorage.getItem('attn_scw')||'',10); if(s>=220&&s<=900) w=s; }catch(e){}
  ...
  function bind(handle, sign){
    ...
    handle.addEventListener('pointermove', e=>{
      if(!drag) return;
      w=Math.max(220, Math.min(900, sw+sign*(e.clientX-sx)));
      root.style.setProperty('--scw', w+'px');
    });
...
(function(){
  ...
  const NODES=[
    ...
    {id:'xor',  nm:'🧩 XOR Problemi', tier:2, v:30, pre:['akt'], tab:'xor',
      d:'Tek bir doğrunun çözemediği en basit problem. Perceptron\'u durduran duvar — ve iki nöronun o duvarı nasıl aştığı. Lineer modelden sinir ağına geçişin tam kırılma noktası.',
      sci:'Rosenblatt\'ın Perceptron\'u (1958) manşet oldu; Minsky & Papert "Perceptrons" kitabında (1969) tek katmanın XOR\'u çözemeyeceğini kanıtlayınca alan 15 yıl dondu. Duvarı aşan fikir — çok katman + geri yayılım — 1986\'da geldi (Rumelhart, Hinton & Williams).',
      real:[['🚨','Dolandırıcılık tespiti: "ya A ya B ama ikisi birden değil" desenleri'],['⚕️','Tıpta etkileşimli risk (iki ilaç ayrı ayrı güvenli, birlikte değil)'],['🖼️','Görüntü tanıma: hiçbir sınıf düz çizgiyle ayrılmaz'],['🧬','Derin öğrenmenin var olma sebebi: doğrusal-olmayan sınırlar']],
      sub:['XOR nedir, neden tek çizgi yetmez','h₁=OR, h₂=AND ayrıştırması','y = h₁ VE (DEĞİL h₂) çözümü','Yeni uzayda lineer ayrılabilirlik','Elle 1 epoch: ileri → hata → gradyan → güncelle','Kendini test (5 soru)']},
    {id:'nn',   nm:'🧠 Neural Network', tier:2, v:62, pre:['lin','xor'], tab:'neural',
...
(function(){
  ...
  const MOD_SCENES = {
    ...
    "ytmod-4": [{n:1, id:"yts-4-1", label:"🎯 Bir cümleyle: zincir kuralı = yol üstündeki oranları çarp"}, {n:2, id:"yts-4-2", label:"🔗 Sahne 1 — Canlı örnek: y=(2x+1)³'ün teğeti"}, {n:3, id:"yts-4-3", label:"💱 Sahne 2 — Döviz zinciri: TL → EUR → USD → Altın"}, {n:4, id:"yts-4-4", label:"🌡️ Sahne 3 — Sigmoid'in türevi: dışın türevi × içinin türevi (canlı)"}, {n:5, id:"yts-4-5", label:"📝 Yazıya dökmek — kalıp örnekler"}],
  ...
  function openScene(sid){
    ...
    // "Sahne N" bloğu kendi de kapanır/açılırsa (varsayılan kapalı),
    // scroll'dan önce onu da aç — aksi halde gizli bir kutuya scroll eder.
    const sHead = sc.querySelector(':scope > .sahne-head');
    const sBody = sc.querySelector(':scope > .sahne-body');
    if(sHead) sHead.classList.remove('closed');
    if(sBody) sBody.classList.remove('closed');
    requestAnimationFrame(() => requestAnimationFrame(() => {
...
(function(){
  ...
  const BOOST={
    ...
    xor:{q:'XOR\'u ayırmak için en az kaç doğru gerekir?', a:'İki — tek doğru dört noktanın en fazla üçünü doğru ayırabilir.'},
